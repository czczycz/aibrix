...
// TestIsVideoListRequest verifies the public catalog is answered only for a bare
// GET /v1/videos: the model query parameter is no longer required (the catalog
// comes from the registry, not from a per-model fan-out). The optional trailing
// slash accepted by the Envoy route must select the same catalog.
func TestIsVideoListRequest(t *testing.T) {
	assert.True(t, isVideoListRequest(PathVideos, http.MethodGet))
	assert.True(t, isVideoListRequest(PathVideos+"?model=wan2.1", http.MethodGet))
	assert.True(t, isVideoListRequest(PathVideos+"/", http.MethodGet))
	assert.True(t, isVideoListRequest(PathVideos+"/?model=wan2.1", http.MethodGet))
...
func newTestVideoJobServer(t *testing.T) (*Server, *MockCache, *AsyncJobRegistry) {
	t.Helper()
	mockCache := new(MockCache)
	registry := newAsyncJobRegistry(newInMemoryAsyncJobStore(), mockCache)
...
func videoJobRecordCount(t *testing.T, registry *AsyncJobRegistry, owner string) int {
	t.Helper()
	records, err := registry.List(context.Background(), owner, asyncJobTypeVideo)
	require.NoError(t, err)
	return len(records)
}
...
func TestHandleVideoJobSubResourceHeaders_StoreUnavailableReturns503(t *testing.T) {
	...
	s := &Server{cache: mockCache, asyncJobs: newAsyncJobRegistry(store, mockCache)}
	ctx := context.Background()
...
func TestHandleVideoListHeaders_ReturnsOwnerScopedPublicCatalog(t *testing.T) {
	...
	resp := s.handleVideoListHeaders(ctx, "req-1", "scope:user:alice")
...
func TestHandleVideoListHeaders_StoreUnavailableReturns503(t *testing.T) {
	mockCache := new(MockCache)
	s := &Server{cache: mockCache, asyncJobs: newAsyncJobRegistry(storeUnavailableAsyncJobStore{}, mockCache)}

	resp := s.handleVideoListHeaders(context.Background(), "req-1", asyncJobOwnerShared)
...
func TestHandleVideoJobResponseBody_RegistersCreateAndRewritesID(t *testing.T) {
	...
	records, err := registry.List(ctx, "scope:user:alice", asyncJobTypeVideo)
	require.NoError(t, err)
...
func TestHandleVideoJobResponseBody_AppliesBackendExpiry(t *testing.T) {
	...
	records, err := registry.List(ctx, asyncJobOwnerShared, asyncJobTypeVideo)
	require.NoError(t, err)
	require.Len(t, records, 1)
...
// TestHandleVideoJobResponseBody_TrailingSlashKeepsIDsOpaque covers the
// optional slash form configured in Envoy. It must take the same buffered,
// rewrite path as the canonical Videos URLs.
func TestHandleVideoJobResponseBody_TrailingSlashKeepsIDsOpaque(t *testing.T) {
	ctx := context.Background()

	t.Run("create registers and rewrites", func(t *testing.T) {
		s, _, registry := newTestVideoJobServer(t)
		requestID := "req-create-slash"
		t.Cleanup(func() { requestBuffers.Delete(requestID) })
		routerCtx := types.NewRoutingContext(ctx, "", "wan2.1", "", requestID, "")
		routerCtx.ReqPath = PathVideos + "/"
		routerCtx.ReqHeaders = map[string]string{methodKey: http.MethodPost}
		...
		resp, complete, handled := s.handleVideoJobResponseBody(ctx, requestID, routerCtx, &extProcPb.ProcessingRequest_ResponseBody{
			ResponseBody: &extProcPb.HttpBody{Body: []byte(`{"id":"video_gen_abc"}`), EndOfStream: true},
		})
	...
	for _, method := range []string{http.MethodGet, http.MethodDelete} {
		t.Run(method+" rewrites", func(t *testing.T) {
			requestID := "req-" + strings.ToLower(method) + "-slash"
			t.Cleanup(func() { requestBuffers.Delete(requestID) })
			s, _, _ := newTestVideoJobServer(t)
			routerCtx := types.NewRoutingContext(ctx, "", "wan2.1", "", requestID, "")
			routerCtx.ReqPath = PathVideos + "/aibrixjob-public/"
			routerCtx.ReqHeaders = map[string]string{methodKey: method}
			...
			assert.True(t, complete)
			out := string(resp.GetResponseBody().GetResponse().GetBodyMutation().GetBody())
			assert.Equal(t, "aibrixjob-public", gjson.Get(out, "id").String())
			assert.NotContains(t, out, "video_gen_abc")
		})
	}
}
