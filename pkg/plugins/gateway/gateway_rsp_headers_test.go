...
func TestHandleResponseHeaders_VideoJSONDropsContentLength(t *testing.T) {
	tests := []struct {
		...
		{"create response with trailing slash is rewritten", http.MethodPost, PathVideos + "/", true},
		{"status response is rewritten", http.MethodGet, PathVideos + "/aibrixjob-abc", true},
		{"status response with trailing slash is rewritten", http.MethodGet, PathVideos + "/aibrixjob-abc/", true},
		...
		{"delete response with trailing slash is rewritten", http.MethodDelete, PathVideos + "/aibrixjob-abc/", true},
	}
