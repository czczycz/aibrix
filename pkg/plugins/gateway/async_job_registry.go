...
func (r AsyncJobRecord) validate() error {
	switch {
	case r.PublicJobID == "":
		return fmt.Errorf("%w: public job id is required", errAsyncJobInvalidRecord)
	case r.JobType == "":
		return fmt.Errorf("%w: job type is required", errAsyncJobInvalidRecord)
	case r.Owner == "":
		return fmt.Errorf("%w: owner is required", errAsyncJobInvalidRecord)
	case r.BackendJobID == "":
		return fmt.Errorf("%w: backend job id is required", errAsyncJobInvalidRecord)
	case r.RoutingTarget.Kind != asyncJobTargetKindPod:
		return fmt.Errorf("%w: unsupported routing target kind %q", errAsyncJobInvalidRecord, r.RoutingTarget.Kind)
	case r.RoutingTarget.Pod.Name == "":
		return fmt.Errorf("%w: routing target pod name is required", errAsyncJobInvalidRecord)
	case r.RoutingTarget.Pod.Namespace == "":
		return fmt.Errorf("%w: routing target pod namespace is required", errAsyncJobInvalidRecord)
	case r.RoutingTarget.Pod.UID == "":
		return fmt.Errorf("%w: routing target pod uid is required", errAsyncJobInvalidRecord)
	case r.ExpiresAt.IsZero():
		return fmt.Errorf("%w: expiry is required", errAsyncJobInvalidRecord)
	}
	return nil
}
...
func newAsyncJobRegistry(store asyncJobStore, pods podResolver) *AsyncJobRegistry {
...
func (r *AsyncJobRegistry) Register(ctx context.Context, reg AsyncJobRegistration) (AsyncJobRecord, error) {
	if reg.Pod == nil {
		return AsyncJobRecord{}, fmt.Errorf("%w: routing target pod is required", errAsyncJobInvalidRecord)
	}
	...
	if !expiresAt.After(now) {
		return AsyncJobRecord{}, fmt.Errorf("%w: expiry %s has already elapsed", errAsyncJobInvalidRecord, expiresAt.UTC().Format(time.RFC3339))
	}
...
// the job is gone.
func (r *AsyncJobRegistry) Get(ctx context.Context, owner, publicJobID string) (AsyncJobRecord, *v1.Pod, error) {
...
// List returns one owner's records of one job type. It is a registry read only:
// no pod is resolved and no backend is contacted, so a listing never reflects
// live job status.
func (r *AsyncJobRegistry) List(ctx context.Context, owner, jobType string) ([]AsyncJobRecord, error) {
	return r.store.list(ctx, owner, jobType)
}

// Delete forgets a job. It is idempotent, so the cleanup that follows a backend
// delete can be retried.
func (r *AsyncJobRegistry) Delete(ctx context.Context, owner, publicJobID string) error {
...
func (r *AsyncJobRegistry) resolvePin(record AsyncJobRecord) (*v1.Pod, error) {
...
// re-running it for the same public id overwrites the record and SADDs a member
// that is already there, so a retried registration cannot duplicate a listing.
//
...
for i = 1, #indexes do
  redis.call('SREM', indexes[i], ARGV[1])
...
func (s *redisAsyncJobStore) put(ctx context.Context, record AsyncJobRecord) error {
	...
	if ttl <= 0 {
		return fmt.Errorf("%w: expiry %s has already elapsed", errAsyncJobInvalidRecord, record.ExpiresAt)
	}
	payload, err := sonic.Marshal(record)
	if err != nil {
		return fmt.Errorf("%w: marshal record: %v", errAsyncJobInvalidRecord, err)
	}
	...
	return retryAsyncJobStoreOp(ctx, s.retry, "put", func(ctx context.Context) error {
		return asyncJobRegisterScript.Run(ctx, s.client, keys,
			record.PublicJobID, payload, ttl.Milliseconds()).Err()
	})
}
...
func (s *redisAsyncJobStore) list(ctx context.Context, owner, jobType string) ([]AsyncJobRecord, error) {
	...
	var members []string
	if err := retryAsyncJobStoreOp(ctx, s.retry, "list_index", func(ctx context.Context) error {
		var err error
	...
	if len(members) > asyncJobRedisMaxIndexLen {
		klog.InfoS("async job index exceeds safe limit, capping to prevent ext_proc timeout",
			"owner", owner, "job_type", jobType, "total", len(members), "capped_to", asyncJobRedisMaxIndexLen)
		members = members[:asyncJobRedisMaxIndexLen]
	}

	keys := make([]string, 0, len(members))
	for _, member := range members {
		if !isValidPublicJobID(member) {
			klog.V(4).InfoS("skipping invalid public job id in index",
				"owner", owner, "job_type", jobType, "member", member)
			continue
		}
		keys = append(keys, asyncJobRecordKey(owner, member))
	}
	if len(keys) == 0 {
		return nil, nil
	}
	...
	if err := retryAsyncJobStoreOp(ctx, s.retry, "list_records", func(ctx context.Context) error {
		var err error
	...
	for i, payload := range payloads {
		...
		if !ok {
			// The record's own TTL fired before the index's did.
			stale = append(stale, members[i])
			continue
		}
		record, err := decodeAsyncJobRecord([]byte(text))
		if err != nil {
			klog.V(4).InfoS("dropping undecodable async job record",
				"owner", owner, "job_type", jobType, "public_job_id", members[i], "error", err)
			stale = append(stale, members[i])
			continue
		}
		if record.JobType != jobType || record.expired(now) {
			stale = append(stale, members[i])
			continue
		}
		records = append(records, record)
	...
	if len(stale) > 0 {
		// Best effort: a listing that succeeded must not fail because the index
		// could not be tidied, and the next listing will try again. Cleanup still
		// uses the store's deadline and retry policy: an expired index member must
		// not let an otherwise successful List outlive its ext_proc budget.
		if err := retryAsyncJobStoreOp(ctx, s.retry, "list_prune", func(ctx context.Context) error {
			return s.client.SRem(ctx, indexKey, stale...).Err()
		}); err != nil {
			klog.V(4).InfoS("failed to prune stale async job index members",
				"owner", owner, "job_type", jobType, "error", err)
		}
	}
...
func decodeAsyncJobRecord(payload []byte) (AsyncJobRecord, error) {
	...
	if err := sonic.Unmarshal(payload, &record); err != nil {
		return AsyncJobRecord{}, fmt.Errorf("%w: unmarshal record: %v", errAsyncJobInvalidRecord, err)
	}
	return record, nil
}
