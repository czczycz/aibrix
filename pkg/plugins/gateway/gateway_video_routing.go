...
// canonicalVideoPath removes a query string and the optional trailing slash
// accepted by the Videos route. It is only for matching; rewriteVideoPathID
// keeps the path the client sent so the backend sees the same slash form.
func canonicalVideoPath(requestPath string) string {
	return strings.TrimSuffix(pathWithoutQuery(requestPath), "/")
}
...
// GET /v1/videos. Unlike the pre-registry fan-out this needs no model query
// parameter: the registry knows every job the caller owns, across models.
...
// videoListResponse is the OpenAI-shaped list envelope. There is no pagination
// in v1: the catalog is one owner's live jobs, which is a small set.
...
func (s *Server) handleVideoListHeaders(ctx context.Context, requestID, owner string) *extProcPb.ProcessingResponse {
	records, err := s.asyncJobRegistry().List(ctx, owner, asyncJobTypeVideo)
	if err != nil {
	...
	items := make([]videoListItem, 0, len(records))
	...
	respBody, err := sonic.Marshal(videoListResponse{Object: "list", Data: items})
	if err != nil {
	...
	klog.InfoS("video job list served from registry", "requestID", requestID, "owner", owner, "jobs", len(items))
