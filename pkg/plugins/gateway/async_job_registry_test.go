...
func TestRedisAsyncJobStore_ListPrunesStaleIndexMembers(t *testing.T) {
	...
	members, err := client.SMembers(ctx, asyncJobIndexKey(asyncJobOwnerShared, asyncJobTypeVideo)).Result()
	require.NoError(t, err)
	assert.Equal(t, []string{"job-1"}, members)
}

// asyncJobPruneHook injects failures after List has assembled its result, so
// the test can assert the stale-index cleanup policy independently of reads.
type asyncJobPruneHook struct {
	attempts int
	fail     func(context.Context) error
}

func (h *asyncJobPruneHook) DialHook(next redis.DialHook) redis.DialHook { return next }

func (h *asyncJobPruneHook) ProcessHook(next redis.ProcessHook) redis.ProcessHook {
...
func (h *asyncJobPruneHook) ProcessPipelineHook(next redis.ProcessPipelineHook) redis.ProcessPipelineHook {
	return next
}

// TestRedisAsyncJobStore_ListStalePruningUsesBoundedRetries keeps cleanup
// best-effort without allowing it to escape the registry's retry deadline.
func TestRedisAsyncJobStore_ListStalePruningUsesBoundedRetries(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "blocked cleanup is deadline bounded",
		...
		{
			name:         "transient cleanup uses retry budget",
			deadline:     80 * time.Millisecond,
			fail:         func(context.Context) error { return io.EOF },
			wantAttempts: asyncJobStoreMaxAttempts,
		},
		{
			name:         "permanent cleanup is not retried",
			deadline:     80 * time.Millisecond,
			fail:         func(context.Context) error { return permanent },
			wantAttempts: 1,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			store := newRedisAsyncJobStore(client)
			store.now = clock.Now
			store.retry = asyncJobRetryPolicy{
				maxAttempts: asyncJobStoreMaxAttempts,
				baseBackoff: time.Millisecond,
				deadline:    tt.deadline,
			}

			record := testAsyncJobRecord(clock, asyncJobOwnerShared, "job-stale", "backend-stale")
			require.NoError(t, store.put(ctx, record))
			mr.Del(asyncJobRecordKey(asyncJobOwnerShared, record.PublicJobID))

			hook := &asyncJobPruneHook{fail: tt.fail}
			client.AddHook(hook)
			started := time.Now()
			listed, err := store.list(ctx, asyncJobOwnerShared, asyncJobTypeVideo)
			elapsed := time.Since(started)

			require.NoError(t, err, "a stale-index cleanup failure must remain best-effort")
			assert.Empty(t, listed)
			assert.Equal(t, tt.wantAttempts, hook.attempts)
			assert.Less(t, elapsed, 2*tt.deadline, "cleanup must not outlive the store deadline")
		})
	}
}
...
func newAsyncJobRegistryFixture(t *testing.T) *asyncJobRegistryFixture {
	...
	registry := newAsyncJobRegistry(store, mockCache)
...
func TestAsyncJobRegistry_GetExpiredRecordIsNotFound(t *testing.T) {
	...
	listed, err := fixture.registry.List(ctx, asyncJobOwnerShared, asyncJobTypeVideo)
	require.NoError(t, err)
	assert.Empty(t, listed)
}
...
func TestAsyncJobRegistry_ListIsScopedAndTyped(t *testing.T) {
	...
	listed, err := fixture.registry.List(ctx, alice, asyncJobTypeVideo)
	require.NoError(t, err)
...
func TestRedisAsyncJobStore_DeleteReapsExpiredRecordIndexOfAnyJobType(t *testing.T) {
	...
	require.NoError(t, store.delete(ctx, asyncJobOwnerShared, "job-audio"))
	members, err := client.SMembers(ctx, asyncJobIndexKey(asyncJobOwnerShared, asyncJobTypeTestAudio)).Result()
	require.NoError(t, err)
...
func TestAsyncJobRegistry_RegisterRejectsElapsedBackendExpiry(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			records, listErr := fixture.registry.List(ctx, asyncJobOwnerShared, asyncJobTypeVideo)
